                        ...
                        unsigned char end, unsigned char end0)
{
    ...
    cprintf(is_fr() ? " HAB %u " : " SKL %u ", (unsigned)hab);
    put_gauge(end, end0);
    ...
    revers(end < 5);
    cprintf(" %u/%u", (unsigned)end, (unsigned)end0);
    revers(0);
}
...
{
    gotoxy(11, row);
    cprintf("%s %u + %u + %u = %u", who, (unsigned)d1, (unsigned)d2,
            (unsigned)(force - d1 - d2), (unsigned)force);
    pad_to(79);
}
...
static void show_fighters(void)
{
    ...
    if (app.foe_count > 1) cprintf(" %u/%u", (unsigned)(app.foe_cur + 1),
...
static int run_combat(void)
{
    ...
    /* "c'est le heros qui a touche" : lu six fois par assaut, dans l'invite,
     * le bruitage, la blessure et le duel au premier sang. Un octet local
    ...
    for (;;) {
        ...
        if (pending && app.hero.cha) {
            put_tag("C");
            cprintf(msg(M_K_ENJEU), (unsigned)app.hero.cha,
                    (unsigned)wgood, (unsigned)wbad);
        } else {
        ...
        if (pending) {
            ...
            if (use_luck) {
                ...
                put_verdict(lucky ? (msg(M_CHANCEUX2)) : (msg(M_MALCHANCEUX2)));
                wait_space_at(CHOICE_ROWN, msg(M_K_CONTINUER));
            }
        ...
        gotoxy(40, CHOICE_ROW0 + 2);
        cprintf("%s", hits ? (msg(M_VOUS_L_AVEZ)) : (msg(M_ELLE_VOUS_A)));
